...
export function getChatRoomUserList(req: Request, res: Response) {
  ...
  const sql = `SELECT userID,username, imgUrl FROM user_chatroom uc
...
export function getChatRoomMessageRecord(req: Request, res: Response) {
  const chatRoomID = req.query.chatRoomID;
  const sql = `SELECT m.id, userID, userName, imgUrl, content, createdAt FROM message m 
...
export function addChatRoomMessage(req: Request, res: Response) {
  ...
  db.query(
    sql,
    [chatRoomID, content, userID, new Date()],
    (err: Error, results: postResponse) => {
      if (err) return res.cc(err);
      if (results.affectedRows <= 0) return res.cc("数据库操作失败");
